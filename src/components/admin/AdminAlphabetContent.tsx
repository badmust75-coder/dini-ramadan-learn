...
const AdminAlphabetContent = () => {
  ...
  const handleUpload = useCallback(async (letterId: number, files: FileList) => {
    if (!user?.id) { toast.error('Vous devez être connecté'); return; }
    setIsUploading(true);
    setUploadingLetterId(letterId);
    try {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (contentId: string) => {
      ...
      try {
        ...
        if (bucketPath[1]) {
          await supabase.storage.from('alphabet-content').remove([decodeURIComponent(bucketPath[1])]);
        }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Téléversez des vidéos, PDF ou images pour chaque lettre.
      </p>
      <div className="space-y-3">
        {letters.map((letter) => {
          const letterContents = contents.filter(c => c.letter_id === letter.id);
          const isThisUploading = isUploading && uploadingLetterId === letter.id;
          return (
            <Card key={letter.id}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => { if (e.target.files?.length) handleUpload(letter.id, e.target.files); e.target.value = ''; }}
